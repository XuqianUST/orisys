...
io1 = r'dataset\C1dataset.mat'
datapath = os.path.join(nowloc,io1)
model_save_path_best = os.path.join(nowloc,'/Best_model_c1.pth')
...
scheduler2 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer2,mode='min', factor = 0.9,verbose = True, patience = 100000, cooldown = 1,eps = 1e-8)
    ...
    if  (epoch+1) % 10 == 0:
        with torch.no_grad():
            ...
            if testloss < val_loss_best:
                ...
                scio.savemat('/dataset/testbest_C1.mat',{'valout' : vout, 'y' : vlabel,'sig':sig})
